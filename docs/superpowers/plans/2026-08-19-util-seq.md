...
| `tests/suites/test_util_seq.c` | 10 组行为测试，目标 100% 行 + 分支 |
...
- Create: `tests/suites/test_util_seq.c`
...
创建 `tests/suites/test_util_seq.c`。**只写这 5 组** —— 插值和回绕在 Task 2。
...
```bash
cd /home/stg/platform_ws/COD_UniCFramework
clang-format -i 06_utils/util_seq/util_seq.h 06_utils/util_seq/util_seq.c tests/suites/test_util_seq.c
...
- Modify: `tests/suites/test_util_seq.c`（新增 8 个测试：6 组插值 + 2 组回绕）
...
```bash
cd /home/stg/platform_ws/COD_UniCFramework
clang-format -i 06_utils/util_seq/util_seq.c tests/suites/test_util_seq.c
cmake --build build -j16 2>&1 | grep -cE "error|warning"
cmake --build build-tests -j16 2>&1 | grep -cE "error|warning"
ctest --test-dir build-tests 2>&1 | tail -3
rm -rf /tmp/seqcov
