...
void* IMPL_STM32_GPIO_CreateCtx(GPIO_TypeDef* port, uint16_t pin)
{
     ...
     * succeeds, PLAT_GPIO_Create returns a valid-looking instance, and the NULL is
     * not dereferenced until the first Write — by which point the fault is in the
     * HAL, several layers from the call that supplied the bad port.
     *
     * A pin mask of zero is refused for the same reason: HAL_GPIO_WritePin would
     * write BSRR with no bits set, which is a silent no-op, and a GPIO that accepts
     * every call while driving nothing is the hardest kind of wiring bug to find.
     * Multi-bit masks are allowed through — the ops contract is one pin per
     * instance, but a caller deliberately ganging pins on one port gets what the
     * hardware does, which is to move them together. */
    if (port == NULL || pin == 0u)
    {
        return NULL;
    }
