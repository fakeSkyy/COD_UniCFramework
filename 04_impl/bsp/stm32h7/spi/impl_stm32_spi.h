...
typedef struct
{
    ...
    /**
     * @brief True between a successful cs_assert and its cs_deassert.
     *
     * Means two things at once, and they are deliberately the same flag: CS stays
     * low across transfers, and this device holds the bus claim. Splitting them
     * would allow the state where CS is held but the bus is not, which is exactly
     * the hazard the hold exists to prevent.
     */
    bool cs_held;
 ...
 * @param cs_port  Chip-select GPIO port (e.g. GPIOA), or NULL for a device with
 *                 no chip select — a write-only target that is the only thing on
 *                 its bus, such as an LED strip clocked out of MOSI. The transfer
 *                 calls then drive no CS at all, and @p cs_pin is ignored.
