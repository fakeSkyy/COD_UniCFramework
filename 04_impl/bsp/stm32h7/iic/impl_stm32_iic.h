 ...
 * outlives it, the same as the SPI backend: iic_bus_acquire is idempotent
 * per handle, so re-creating a device on the same @p hi2c finds the existing
 * record rather than accumulating a new one. Nothing here needs to touch it.
 *
