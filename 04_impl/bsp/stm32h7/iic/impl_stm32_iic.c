...
/* Every one of these claims the bus for the duration of the call.
 *
 * They did not, which made the arbitration above a half-measure: an asynchronous
 * transfer would claim the peripheral correctly, and then a blocking call on a second
 * device sharing it would walk straight into the HAL and start a transfer on top. The
 * HAL refuses that with HAL_BUSY, so the blocking call returns false — but a blocking
 * call issued FIRST left the bus unclaimed, so an async transfer could be started
 * underneath it, and that one does move the owner pointer. The in-flight device's
 * completion is then delivered to the wrong context.
 *
 * Claiming here also gives the two paths one meaning of "busy" rather than two, which
 * is what makes the guard in iic_begin trustworthy from either side. The SPI backend
 * already worked this way; this brings I2C in line with it. */
...
static bool stm32_iic_is_ready(void* ctx, uint32_t trials, uint32_t timeout)
{
    ...
    /* Address probing drives the bus like any other transfer, so it is arbitrated
     * too — a probe issued while another device is mid-transfer would otherwise put a
     * START and an address onto a bus already carrying one. */
    if (!iic_begin(c))
    {
        return false;
    }
                                      ...
                                      const uint8_t* data, uint16_t len)
{
    ...
    /* Checked before the bus is claimed, so a rejected call does not have to release
     * it. A zero length is refused rather than passed down: the HAL treats it as a
     * transfer that completes with no data, so the completion callback fires and the
     * caller sees a successful transfer that moved nothing. */
    if (data == NULL || len == 0u || !iic_begin(c))
    {
        return false;
    }
...
static bool stm32_iic_seq_transfer(void* ctx, const IIC_Seq_Step_s* steps, uint8_t count)
{
    ...
    {
        if (steps[i].data == NULL || steps[i].len == 0u)
        {
            return false;
        }
    }
...
#define IIC_THUNKS(n)                                                                              \
    ...
    static void iic##n##_err(I2C_HandleTypeDef* h)                                                 \
    {                                                                                              \
        if (iic_buses[n].hi2c == h)                                                                \
        {                                                                                          \
            iic_on_error(iic_buses[n].active, h);                                                  \
        }                                                                                          \
    }
